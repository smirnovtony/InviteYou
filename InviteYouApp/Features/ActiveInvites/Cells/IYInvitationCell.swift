...
class IYInvitationCell: UITableViewCell {
    ...
    // создание идетификатора ячейки. Обязательно static, и имя - как имя класса
    static let reuseIdentifier: String = "IYInvitationCell"

    // настройка всей ячейки
    ...
    private lazy var logoView: UIImageView = {
        ...
        imageView.image = UIImage(named: "photography") // дэфолтная картинка, сюда логотип организации
    ...
    private lazy var dataLable: UILabel = {
        ...
        label.backgroundColor = mainСolorGreen // сделать его меняющимся в зависимости от даты: красный если прошло или отменено, зеленый если принял, желтый если не знаешь
    ...
    override init(style: IYInvitationCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.initCell() // инициализация ячейки
    }
    ...
    func initCell() { // добавление на contentView (то что есть на ячейка добаляется на contentView), а лучше на cardContainerView как на подложку
...
//        self.backgroundColor = selected ? .lightGray : .white // фон ячейки при наведении
    ...
    override func updateConstraints() {
        ...
        self.organizerLable.snp.makeConstraints { (make) in
            ...
            make.right.equalTo(self.dataLable.snp.left).offset(-10)
...
//        self.logoView // как передать изобаржение??
        self.organizerLable.text = organizerLable
        self.addressLable.text = addressLable
